...
beta_a <- array(rnorm(n_voxel * n_subject, sd = 0.05),
                c(n_voxel, n_subject, 1L))
beta_b <- array(rnorm(n_voxel * n_subject, sd = 0.05),
                c(n_voxel, n_subject, 1L))
variance <- array(0.05^2, dim(beta_a))

signal <- array(0, dims3)
signal[15:18, 15:18, 8:9] <- -0.8
signal[16:17, 16:17, 8:9] <- 0.8
...
for (i in seq_len(n_subject)) {
  ...
  beta_a[, i, 1L] <- truth + rnorm(n_voxel, sd = 0.05)
  beta_b[, i, 1L] <- truth + rnorm(n_voxel, sd = 0.05)
}
